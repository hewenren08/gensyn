...
start_training() {
    ...
    # 检查日志中是否有DHT连接错误或其他严重错误
    if [ -f "$ROOT/logs/yarn.log" ]; then
        # 检查是否有典型的错误模式
        if grep -q "TypeError: cannot unpack non-iterable NoneType object" "$ROOT/logs/yarn.log" 2>/dev/null; then
            echo_red ">> Detected DHT connection error in logs"
            return 1  # 强制返回错误码
        fi
        
        if grep -q "Exception occurred during game run" "$ROOT/logs/yarn.log" 2>/dev/null; then
            echo_red ">> Detected game runtime exception in logs"
            return 1  # 强制返回错误码
        fi
    fi
